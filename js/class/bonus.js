class Bonus {
  constructor(CTX, canvasDOM, bonusNum, speedY, floorPosY, bonusID) {
    ...
    this.position = {
      Y: -this.size.width,
      X: Math.random() * (this.canvasDOM.width - this.size.width * 3),
    };
    this.img = undefined;
  }
  ...
  activateBonus(player) {
    switch (this.bonusNum) {
      ...
      case 5: //FREEZE
        ...
        break;
      case 6: //PARTY HARD
        gameManager.numballons = 20;
        gameManager.balloonsSpawnTime = 10;
        setTimeout(() => {
          gameManager.numballons = 7;
          gameManager.balloonsSpawnTime = 30;
        }, 6000);
    }
  }
  checkBorderDown() {
    ...
    if (
      ...
      this.canvasDOM.height - this.floorPosY
    ) {
      return true;
    } else {
      return false;
